#!/usr/bin/env python3
"""
Instagram Slides Renderer (libreria) - saludmentalcostarica.com
Paleta: Teal (#2B7073), Coral (#FB7A62), Crema (#F6EFDF)
Backgrounds: arte abstracto generativo con numpy, evocativo del contenido.

Derivado de vendor/instagram-slides/scripts/create_slides.py, convertido de script
CLI a libreria. Expone:

    render_carousel(spec: dict) -> list[tuple[str, bytes]]

que devuelve [(filename, png_bytes), ...] en memoria (BytesIO), sin escribir a disco.
Unico efecto de sistema de archivos: tempfiles para imagenes descargadas de Storage,
...
# Resampling.LANCZOS en Pillow >= 9.1; fallback al alias antiguo.
try:
...
# -- Fuentes ----------------------------------------------------------------
# Poppins empaquetada junto al codigo (api/slides/_assets/fonts/), con fallback
# a DejaVu del sistema y, en ultimo caso, la fuente por defecto de Pillow.
_FONTS_DIR = os.path.normpath(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "_assets", "fonts")
)
_DEJAVU_DIRS = [
...
def F(size, bold=False, light=False):
    ...
    if bold:
        names, dv = ["Poppins-Bold.ttf"], "DejaVuSans-Bold.ttf"
    elif light:
        names, dv = ["Poppins-Light.ttf", "Poppins-Regular.ttf"], "DejaVuSans.ttf"
    else:
        names, dv = ["Poppins-Regular.ttf", "Poppins-Medium.ttf"], "DejaVuSans.ttf"

    fnt = None
    # 1) Poppins empaquetada
    for n in names:
        p = os.path.join(_FONTS_DIR, n)
        if os.path.exists(p):
            fnt = ImageFont.truetype(p, size)
            break
    # 2) DejaVu del sistema (rutas conocidas)
    if fnt is None:
        for d in _DEJAVU_DIRS:
            ...
            if os.path.exists(p):
                fnt = ImageFont.truetype(p, size)
                break
    # 3) DejaVu por nombre (busqueda interna de Pillow)
    if fnt is None:
        try:
            fnt = ImageFont.truetype(dv, size)
        except OSError:
            fnt = None
    # 4) Ultimo recurso: fuente por defecto de Pillow (dimensionable en >=10.1)
    if fnt is None:
        try:
            fnt = ImageFont.load_default(size)
        except TypeError:  # Pillow antiguo: sin argumento de tamano
...
# ---------------------------------------------------------------------------
# IMAGENES (local o Supabase Storage)
# ---------------------------------------------------------------------------
...
def supabase_config():
    """
    Devuelve (base_url, service_role_key) para operaciones server-side de Storage.
    Acepta SUPABASE_URL o, como fallback, el NEXT_PUBLIC_SUPABASE_URL que ya usa el
    CRM, para no obligar a duplicar la variable en Vercel.
    """
    base = (
...
def _download_storage_object(storage_key: str) -> str:
    """
    Descarga un objeto de Supabase Storage a un tempfile y devuelve su path.
    storage_key: "carousel-images/<path>" (bucket incluido como primer segmento).
    Usa SUPABASE_URL/NEXT_PUBLIC_SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY del entorno.
    """
    import requests

    base, service_key = supabase_config()
    if not base or not service_key:
        raise RuntimeError(
            "SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY no configurados: "
            "no se puede resolver una imagen 'storage:'"
        )

    url = f"{base}/storage/v1/object/{storage_key.lstrip('/')}"
    resp = requests.get(
        url,
    ...
    suffix = os.path.splitext(storage_key)[1] or ".img"
    fd, path = tempfile.mkstemp(prefix="slide_img_", suffix=suffix)
    with os.fdopen(fd, "wb") as fh:
        fh.write(resp.content)
    return path

def _resolve_image_field(slide: dict, tempfiles: list) -> dict:
    """
    Si slide["image"] es una clave "storage:carousel-images/<path>", la descarga a
    un tempfile y devuelve una copia del slide con "image" apuntando al path local.
    Paths locales se dejan tal cual. Registra los tempfiles para limpieza posterior.
    """
    img = slide.get("image", "")
    if not img or not img.startswith(STORAGE_PREFIX):
        return slide
    storage_key = img[len(STORAGE_PREFIX):]  # p.ej. "carousel-images/2026-07/uuid.jpg"
    local = _download_storage_object(storage_key)
    tempfiles.append(local)
    resolved = dict(slide)
...
def load_photo(path, size, style="duotone"):
    ...
    ph = ImageOps.fit(ph, size, method=_LANCZOS, centering=(0.5, 0.42))
    if style == "duotone":
...
# Vocabulario visual por tema (sin acentos: el match se hace normalizado).
STYLE_KEYWORDS = {
...
SLIDE_TYPES = tuple(CREATORS.keys())


def render_carousel(spec: dict):
    """
    Renderiza un carrusel completo en memoria.

    spec: {"title": str (opcional), "slides": [ {"type": ..., ...}, ... ]}
    Devuelve: [(filename, png_bytes), ...] en el orden de las slides.

    No escribe a disco salvo tempfiles para imagenes "storage:" (se limpian aqui).
    """
