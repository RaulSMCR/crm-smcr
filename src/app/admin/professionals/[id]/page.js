...
export default function AdminProfessionalDetailPage() {
  ...
  async function load() {
    ...
    try {
      const res = await fetch(`/api/admin/professionals/${id}`, { method: "GET" });
      const data = await res.json().catch(() => ({}));

      if (!res.ok) throw new Error(data?.error || data?.message || "No se pudo cargar el profesional.");
          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-2 text-sm text-neutral-700">
              <div>
                ...
                {prof.createdAt ? new Date(prof.createdAt).toLocaleString() : "—"}
              </div>

              <div>
                <span className="font-medium">Última actualización:</span>{" "}
                {prof.updatedAt ? new Date(prof.updatedAt).toLocaleString() : "—"}
              </div>
