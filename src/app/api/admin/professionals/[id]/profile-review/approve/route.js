...
export async function POST(_request, { params }) {
  try {
    ...
    if (!profile.profileReviewDraft || !profile.profileReviewDraft.trim()) {
      return NextResponse.json({ message: "No hay resena pendiente para aprobar." }, { status: 400 });
    }

    const updated = await prisma.professionalProfile.update({
      where: { id: professionalId },
      data: {
        profileReview: profile.profileReviewDraft.trim(),
    ...
    console.error("approve profile review error:", error);
