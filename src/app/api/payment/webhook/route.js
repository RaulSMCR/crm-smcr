...
async function processAuthenticatedWebhook(request, trace) {
  ...
  if (processedTransaction.status === "APPROVED") {
    ...
    await sendPaymentConfirmationEmail(processedTransaction).catch((error) =>
      logOnvoWebhook("error", "PAYMENT_CONFIRMATION_EMAIL_FAILED", { transactionId: processedTransaction.id, error })
    );
    after(() => submitInvoiceToFe(invoiceId).catch((error) =>
      logOnvoWebhook("error", "FE_SUBMIT_FAILED", { invoiceId, error })
    ));
