...
async function sendPaymentConfirmationEmail({ transaction, isDeposit }) {
  ...
  const body = isDeposit
    ? `<p>Hola ${patientName}, recibimos tu depósito de <strong>${currency} ${amount}</strong> para tu cita con ${proName}. ¡Tu cita está reservada!</p><p>Recuerda que el saldo restante se cobrará al concluir la sesión.</p>`
    : `<p>Hola ${patientName}, tu pago de <strong>${currency} ${amount}</strong> para la cita con ${proName} fue procesado exitosamente. ¡Gracias!</p>`;
...
async function sendPaymentFailedEmail({ transaction }) {
  ...
  const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;color:#0f172a;"><h2>Pago no procesado ❌</h2><p>Hola ${patientName}, tu pago no pudo ser procesado.</p><p><a href="${retryUrl}" style="background:#000;color:#fff;padding:10px 20px;text-decoration:none;border-radius:5px;">Intentar nuevamente</a></p><p style="font-size:12px;color:#64748b;">Este correo fue generado automáticamente.</p></div>`;
