      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl shadow-lg border border-yellow-200 text-center max-w-md">
          ...
          <p className="text-slate-600 mt-2 text-sm">
            Tu cuenta está siendo validada por la administración. 
    ...
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Hola, {profile.user.name.split(' ')[0]} 👋</h1>
          ...
          <div className="flex gap-3 items-center">
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-bold border ${isActive ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
               {isActive ? 'ONLINE' : 'OFFLINE'}
            </span>
          </div>
        </div>

        {/* B. CENTRO DE COMANDO (GRID DE ACCIONES) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            
            {/* 1. Gestión de Perfil y Servicios */}
            <Link href="/panel/profesional/perfil" className="group bg-white p-6 rounded-xl shadow-sm border border-slate-100 hover:shadow-md hover:border-blue-300 transition-all relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity text-6xl text-blue-500">👤</div>
            ...
            {/* 2. Gestión de Blog */}
            <Link href="/panel/profesional/posts" className="group bg-white p-6 rounded-xl shadow-sm border border-slate-100 hover:shadow-md hover:border-purple-300 transition-all relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity text-6xl text-purple-500">✍️</div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                    Gestiona tus publicaciones. ({profile._count.posts} activos)
                </p>
            </Link>

            {/* 3. Gestión de Horarios */}
            <Link href="/panel/profesional/horarios" className="group bg-white p-6 rounded-xl shadow-sm border border-slate-100 hover:shadow-md hover:border-green-300 transition-all relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity text-6xl text-green-500">📅</div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                    Define tu disponibilidad semanal para citas.
                </p>
            </Link>

            {/* 4. Resumen Rápido (No clickeable, solo info) */}
            ...
            {safeAppointments.length === 0 ? (
                ...
                <div className="divide-y divide-slate-50">
                    {safeAppointments.map(appt => (
                        <div key={appt.id} className="p-4 hover:bg-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 transition">
