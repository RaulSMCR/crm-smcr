...
export default async function PacienteAgendarPage({ searchParams }) {
  ...
  if (session.role !== "USER") redirect("/panel"); // panel por rol
  ...
  if (!service || !service.isActive) redirect("/servicios");
  if (!professional || !professional.isApproved || !professional.user?.isActive) redirect("/servicios");
...
export default async function PacienteAgendarPage({ searchParams }) {
  ...
  if (!service || !service.isActive) redirect("/servicios");
  if (!professional || !professional.isApproved || !professional.user?.isActive) redirect("/servicios");
  if (!assignment || assignment.status !== "APPROVED") redirect(`/servicios/${serviceId}`);

  const booked = appts.map((a) => ({ startISO: a.date.toISOString(), endISO: a.endDate.toISOString() }));
