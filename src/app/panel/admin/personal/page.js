...
export default async function AdminPersonnelPage() {
  // Obtenemos profesionales que YA fueron aprobados (isApproved: true)
  const professionals = await prisma.user.findMany({
    where: { role: 'PROFESSIONAL', isApproved: true },
    include: {
      professionalProfile: true,
      _count: {
        select: { appointments: true } // Contamos sus citas para medir rendimiento
      }
    },
    orderBy: { name: 'asc' }
  });
            ...
            {professionals.map((pro) => (
              <tr key={pro.id} className="hover:bg-slate-50 transition">
                ...
                <td className="p-4 text-sm text-slate-600">
                  ...
                  <br/>
                  <span className="text-xs text-slate-400">Mat: {pro.professionalProfile?.licenseNumber || "-"}</span>
                </td>
                <td className="p-4">
                  <span className="bg-blue-50 text-blue-700 px-2 py-1 rounded text-xs font-bold border border-blue-100">
                    {pro._count.appointments} Citas
                  </span>
                </td>
                <td className="p-4">
                  <span className={`px-2 py-1 rounded text-xs font-bold border ${pro.isActive ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
                ...
                <td className="p-4 text-right">
                  <form action={async () => {
                    'use server';
                    // Al rechazar un usuario aprobado, lo desactivamos (Soft Delete)
