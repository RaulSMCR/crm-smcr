...
import { countUniquePatientIds, groupHubAppointments, isHubRaulAttributed } from "@/lib/hub-raul-dashboard";
...
export default async function AcquisitionDashboard({ searchParams }) {
  ...
  const [users, views, completed, leads, hubCompleted] = await Promise.all([
    prisma.user.findMany({ where: { createdAt: { gte: from, lte: to } }, select: { id: true, acquisitionChannel: true, campaignName: true, utmSource: true, utmMedium: true, utmCampaign: true, landingPath: true } }),
    ...
    prisma.lead.findMany({ where: { createdAt: { gte: from, lte: to } }, select: { status: true, channel: true, utmSource: true, utmCampaign: true, landingPath: true } }),
    prisma.appointment.findMany({
      where: {
        status: "COMPLETED",
        date: { gte: from, lte: to },
        professional: { is: { slug: "raul-olmedo" } },
      },
      select: { patientId: true, topicSlug: true, patient: { select: { campaignName: true, utmCampaign: true, landingPath: true } } },
    }),
  ]);
  ...
  const hubUsers = users.filter(isHubRaulAttributed);
  const hubLeads = leads.filter(isHubRaulAttributed);
  const hubAppointments = hubCompleted.filter((appointment) => isHubRaulAttributed(appointment.patient));
  const hubTopics = groupHubAppointments(hubAppointments);
  const hubCompletedPatients = countUniquePatientIds(hubAppointments);
  const hubRecurrentPatients = [...hubAppointments.reduce((counts, row) => counts.set(row.patientId, (counts.get(row.patientId) || 0) + 1), new Map()).values()].filter((count) => count >= 2).length;
  ...
  return <main className="mx-auto max-w-7xl space-y-6 p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><Link href="/panel/admin/marketing" className="text-sm text-slate-500">Volver a marketing</Link><h1 className="mt-2 text-3xl font-bold text-slate-900">Adquisición</h1><p className="text-sm text-slate-600">Resumen de {from.toLocaleDateString("es-CR", { month: "long", year: "numeric" })}.</p></div><form><input type="month" name="month" defaultValue={`${from.getFullYear()}-${String(from.getMonth() + 1).padStart(2, "0")}`} className="rounded-lg border px-3 py-2" /><button className="ml-2 rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white">Aplicar</button></form></div>
    ...
    <section className="rounded-xl border-2 border-brand-200 bg-brand-50 p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wide text-brand-700">Hub de Raúl Olmedo Evans</p><h2 className="mt-1 text-xl font-bold text-slate-900">Conversión atribuida</h2><p className="mt-1 max-w-2xl text-sm text-slate-600">Personas cuyo primer acceso o campaña quedó asociado a <code>/raul-olmedo-evans</code>. Son totales del período seleccionado y no incluyen visitantes anónimos.</p></div><Link href="/raul-olmedo-evans" target="_blank" rel="noreferrer" className="text-sm font-semibold text-brand-700 hover:underline">Ver hub ↗</Link></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["Leads", hubLeads.length], ["Registros", hubUsers.length], ["Pacientes con cita completada", hubCompletedPatients], ["Pacientes recurrentes", hubRecurrentPatients]].map(([label, value]) => <div key={label} className="rounded-lg bg-white p-4"><p className="text-xs uppercase text-slate-500">{label}</p><p className="mt-2 text-2xl font-bold text-slate-900">{value}</p></div>)}</div><div className="mt-5 rounded-lg bg-white p-4"><h3 className="font-semibold text-slate-900">Citas completadas por origen temático</h3><div className="mt-3"><Bars rows={hubTopics} label="hub-raul" /></div></div></section>
