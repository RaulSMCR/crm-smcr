...
export default function ReconciliationPanel() {
  ...
  useEffect(() => { const saved = window.localStorage.getItem("onvoCommissionPct"); if (saved) setRate(Number(saved)); fetch("/api/admin/reconciliation").then((r) => r.json()).then(setData); }, []);
  if (!data) return <p className="text-sm text-slate-500">Cargando conciliación...</p>;
  const processingCost = Number(data.summary?.gross || 0) * rate / 100;
  const expected = Number(data.summary?.gross || 0) - processingCost;
  const difference = received === "" ? null : Number(received) - expected;
  return <div className="space-y-5">
    <div className="grid gap-4 md:grid-cols-4">{[["Pagos aprobados", data.summary.approvedCount], ["Bruto", `₡${data.summary.gross.toLocaleString("es-CR")}`], [`Costo de procesamiento estimado (${rate}%)`, `₡${processingCost.toLocaleString("es-CR")}`], ["Depósito esperado", `₡${expected.toLocaleString("es-CR")}`]].map(([label, value]) => <div key={label} className="rounded-xl border bg-white p-4"><p className="text-xs uppercase text-slate-500">{label}</p><p className="mt-2 text-xl font-bold">{value}</p></div>)}</div>
    <div className="rounded-xl border bg-white p-4"><label className="text-sm font-semibold">Costo ONVO (%)<input value={rate} onChange={(e) => { const next = Number(e.target.value); setRate(next); window.localStorage.setItem("onvoCommissionPct", String(next)); }} type="number" step="0.01" className="ml-3 w-24 rounded-lg border px-3 py-2" /></label><label className="ml-5 text-sm font-semibold">Depósito recibido<input value={received} onChange={(e) => setReceived(e.target.value)} type="number" step="0.01" className="ml-3 rounded-lg border px-3 py-2" /></label>{difference !== null ? <p className={`mt-3 font-semibold ${Math.abs(difference) < 0.01 ? "text-emerald-700" : "text-red-700"}`}>Diferencia: ₡{difference.toLocaleString("es-CR")}</p> : null}</div>
