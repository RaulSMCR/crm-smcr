...
/**
 * Sustitución: cuando ninguna de las 16 candidatas del día sirve, se trae otra
 * de las 1.112 del corpus. El corpus no viaja al navegador; la búsqueda ocurre
 * en el servidor y solo vuelven los resultados.
 */
export default function PhraseSubstitute({ fecha, facetas, elegida }) {
  ...
  function sustituir(indice) {
    setError(null);
    iniciar(async () => {
      const r = await elegirFraseDelDia({ fecha, indice, sustituida: true });
          ...
          {resultados.map((r) => (
            <div
              ...
              className={`rounded-lg border px-4 py-3 ${
                elegida === r.indice ? "border-brand-500 bg-brand-50" : "border-neutral-200 bg-white"
              }`}
            >
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px] text-neutral-600">
                ...
                <button
                  ...
                  disabled={pendiente || elegida === r.indice}
                ...
                >
                  {elegida === r.indice ? "Ya elegida" : "Usar para este día"}
                </button>
              </div>
            </div>
          ))}
