...
export default function ProfileEditor({ profile, allServices = [] }) {
  ...
  // NUEVO: estado cambiar contraseña
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // B. FormData para Server Action updateProfile
  ...
  async function handleChangePassword(e) {
    ...
    if (!pw.currentPassword) {
      setPwMsg({ type: 'error', text: 'Debes ingresar tu contraseña actual.' });
      return;
    }
    ...
    if (pw.newPassword !== pw.confirmPassword) {
      setPwMsg({ type: 'error', text: 'La confirmación no coincide.' });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setPwMsg({ type: 'error', text: data?.error || 'No se pudo actualizar la contraseña.' });
        return;
      }

      setPwMsg({ type: 'success', text: data?.message || 'Contraseña actualizada.' });
      setPw({ currentPassword: '', newPassword: '', confirmPassword: '' });
    } catch (err) {
      setPwMsg({ type: 'error', text: 'Error de red. Intenta de nuevo.' });
    } finally {
      setPwLoading(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="bg-white p-8 rounded-xl shadow-sm border border-slate-100">
        <h3 className="text-lg font-bold text-slate-800 mb-2">Cambiar contraseña</h3>
        <p className="text-slate-500 text-sm mb-6">
          Para tu seguridad, debes ingresar tu contraseña actual.
        </p>

        {pwMsg.text && (
          <div className={`mb-4 p-4 rounded-lg font-medium text-center ${pwMsg.type === 'success' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
            {pwMsg.text}
          </div>
        )}
        ...
        <form onSubmit={handleChangePassword} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="md:col-span-3 flex justify-end pt-2">
            <button
              type="submit"
              disabled={pwLoading}
              className={`px-6 py-3 bg-slate-900 text-white rounded-xl font-bold shadow hover:bg-black transition ${pwLoading ? 'opacity-70 cursor-wait' : ''}`}
            >
