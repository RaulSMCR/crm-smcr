...
const PAYMENT_CONTEXT = {
  appointment: { select: {
    ...
    modality: true, pricePaid: true, priceTierId: true,
...
/**
 * Si el pago llenó un escalón de la escalera de precios, el precio público
 * cambió. Se revalida después de confirmar, nunca dentro de la transacción, y un
 * fallo acá no puede convertir en error un pago ya registrado.
 */
function revalidarSiCambioElPrecio(result) {
  if (!result?.cambiaPrecio) return result;
  try {
    revalidarPreciosPublicos();
  } catch (error) {
    console.error("No se pudieron revalidar los precios públicos:", error?.message);
  }
  return result;
}
...
async function applyPayment(tx, transaction, event, payload, usdCrcRate, manual = false) {
  ...
  // precios: el pago ocupa su cupo y lo inscribe con ese precio en esta misma
  // transacción. Un reintento de ONVO llega como duplicado, así que fuera de
  // acá la inscripción se perdería.
  const escalera = await ocuparCupoEnTransaccion(tx, {
    ...
    serviceId: transaction.appointment?.service?.id ?? null,
    pricePaid: transaction.appointment?.pricePaid,
    priceTierId: transaction.appointment?.priceTierId ?? null,
  });

  return { kind: "processed", transaction: processed, nextPaymentStatus, cambiaPrecio: escalera.cambiaPrecio, ...invoice };
}

export async function processOnvoPayment(prisma, event, payload, { usdCrcRate } = {}) {
  ...
  return revalidarSiCambioElPrecio(result);
}

/** La selección del administrador no permite reutilizar un cobro o resucitar un reembolso. */
export async function reconcileOnvoPayment(prisma, { unmatchedId, transactionId, usdCrcRate }) {
  const result = await serializable(prisma, async (tx) => {
